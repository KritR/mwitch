...
final class SwitcherPanel: NSPanel {
    ...
    func update(entries: [WindowEntry], selection: Int) {
        _ = listState.update(entries: entries, selection: selection)
        sizeToContents()
        renderContent()
    }

    func setSelection(_ index: Int) {
        _ = listState.setSelection(absoluteIndex: index)
        renderContent()
    }
    ...
    private func renderContent(scrollTarget: Int? = nil) {
        hostingView?.rootView = makePanelView(scrollTarget: scrollTarget ?? listState.selectedRow)
    }
